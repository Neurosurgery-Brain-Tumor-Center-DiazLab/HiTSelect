function hh=gname_pareto(cases,line_handle,pdata)
...
%   
%   HH = GNAME(CASES,LINE_HANDLE,pdata) returns a vector of handles
...
%
%   pdata is a structure of metadata to use with ginput_pareto
...
if (length(a) < 2)
   h=gnamesub(dolengthwng,cases,line_handle,[],[],[],[],pdata);
else
   h = [];
   bigax = gca;
   set(figh,'CurrentAx',bigax)
   [x0,y0,x1,y1] = ginput0(pdata);
   while ~isempty(x0)
      % Invoke subroutine with current axes set properly
      [h0,dolengthwng] = gnamesub(dolengthwng,cases,line_handle,x0,y0,x1,y1,pdata);
      h = [h; h0(:)];
...
h = findobj(gca,'Type','text','Tag','gname');
if nargout > 0
   hh = h(ishghandle(h));
end

% ----------------------------------
function [h,dolengthwng]=gnamesub(dolengthwng,cases,line_handle,x0,y0,x1,y1,pdata)
...
if (feature('HGUsingMATLABClasses'))
    scatters = getScatterPlotObject(axesh);
else
...
if isempty(x0),[x0,y0,x1,y1] = ginput0(pdata);end
h = [];
while(~isempty(x0))
   ...
   if ~isempty(idx)               % if any points were selected
      ...
      if adding
         ...
         h0 = text(x0, y0, t0, 'VerticalAlignment', 'baseline', 'Tag','gname','FontSize',12);
         h = [h; h0(:)];
         f = [];
...
function [x0,y0,x1,y1] = ginput0(pdata)
x0 = [];y0 = [];x1 = [];y1 = [];
try 
    [x,y,key] = ginput_pareto(1,pdata);
catch ME
