function [out1,out2,out3] = ginput_pareto(arg1,pdata)
%GINPUT Graphical input from mouse.
%   [X,Y] = GINPUT_PARETO(N,pdata) gets N points from the current axes and returns
...
%
%   [X,Y] = GINPUT_PARETO gathers an unlimited number of points until the
%   return key is pressed.
%
%   [X,Y,BUTTON] = GINPUT_PARETO(N,pdata) returns a third result, BUTTON, that
%   contains a vector of integers specifying which mouse button was
%   used (1,2,3 from left) or ASCII numbers if a key on the keyboard
%   was used.
%
%   pdata:  pdata is metadata for the callback for windowbuttonmotionfcn
%       (the mouse poll callback), which will trap waitforbuttonpress (mouse click event listener). 
%   Examples:
%       [x,y] = ginput;
%
%       [x,y] = ginput(5);
